...
cache_dir=outputs/ebnerd_small_reaction_qwen_text_h6_10k
...
e3_out=outputs/ebnerd_small_future_item_ranking_10k_h6_reaction_qwen_e3
rm -rf "$e3_out"
mkdir -p "$e3_out"
...
e3_joint_out=outputs/ebnerd_small_future_item_ranking_10k_h6_reaction_qwen_e3_joint
rm -rf "$e3_joint_out"
...
r0_out=outputs/ebnerd_small_future_item_ranking_10k_h6_reaction_qwen_r0
rm -rf "$r0_out"
mkdir -p "$r0_out"
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}" /home/robotuser/miniconda3/bin/python -u ebnerd_event_set_predictor.py \
  "${common_args[@]}" \
