...
out=outputs/ebnerd_hstu_jepa_10k_h6_rich_primary_purelist_score_fused_ranker
rm -rf "$out"
mkdir -p "$out"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" /home/robotuser/miniconda3/bin/python -u ebnerd_hstu_jepa.py \
  ...
  --enable-reaction-blend \
  --reaction-blend-lambda-grid 0,0.25,0.5,1,2,3,5,8,10,15,20,50 \
  --enable-rich-fusion-head \
  --fusion-epochs 8 \
  --fusion-patience 3 \
  --fusion-lr 0.003 \
  --fusion-residual-scale 2.0 \
