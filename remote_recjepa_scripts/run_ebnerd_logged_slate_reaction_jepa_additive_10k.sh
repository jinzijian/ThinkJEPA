...
out=outputs/ebnerd_small_logged_slate_reaction_jepa_10k_additive
mkdir -p "$out"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-2}" /home/robotuser/miniconda3/bin/python -u ebnerd_logged_slate_reaction_jepa.py \
  ...
  --out-dir "$out" \
  --reaction-target-cache outputs/ebnerd_small_logged_slate_reaction_jepa_10k/logged_reaction_targets \
