...
out=outputs/ebnerd_hstu_jepa_10k_h6_rich_lgbm_rankopt
rm -rf "$out"
mkdir -p "$out"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" /home/robotuser/miniconda3/bin/python -u ebnerd_hstu_jepa.py \
  ...
  --pretrain-event-weight 0.20 \
  --pretrain-reaction-weight 5.0 \
  --rich-state-weight 1.0 \
  --rich-item-weight 0.25 \
  ...
  --jepa-weight 0.01 \
  --reaction-weight 1.0 \
  --reaction-bce-weight 0.5 \
  --reaction-listwise-weight 5.0 \
  ...
  --enable-lgbm-reranker \
  --no-lgbm-binary-labels \
  --lgbm-gain-scale 10 \
  --lgbm-max-gain-label 20 \
  --lgbm-estimators 800 \
  --lgbm-learning-rate 0.025 \
  --lgbm-num-leaves 63 \
  --lgbm-min-child-samples 30 \
  --lgbm-reg-lambda 0.5 \
  --lgbm-early-stopping-rounds 80 \
  2>&1 | tee "$out/run.log"
