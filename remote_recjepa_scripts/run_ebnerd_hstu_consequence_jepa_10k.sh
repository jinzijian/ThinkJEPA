...
out=outputs/ebnerd_hstu_consequence_jepa_10k_h6_current_action
rm -rf "$out"
mkdir -p "$out"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" /home/robotuser/miniconda3/bin/python -u ebnerd_hstu_jepa.py \
  ...
  --pretrain-consequence-weight 1.0 \
  --rich-state-weight 0.0 \
  --rich-item-weight 0.0 \
  --epochs 3 \
  --jepa-ranker-epochs 3 \
  --patience 2 \
  --batch-size 64 \
  --d-model 128 \
  --heads 4 \
  ...
  --jepa-weight 0.50 \
  --reaction-weight 0.50 \
  --consequence-weight 1.0 \
  --consequence-neg-weight 0.35 \
  --reaction-bce-weight 0.2 \
