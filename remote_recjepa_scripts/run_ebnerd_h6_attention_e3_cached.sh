...
out=outputs/ebnerd_small_future_item_ranking_10k_h6_attention_e3_cached
rm -rf "$out"
mkdir -p "$out"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}" /home/robotuser/miniconda3/bin/python -u ebnerd_event_set_predictor.py \
  ...
  --loss-variants "" \
  --load-predicted-slots outputs/ebnerd_small_future_item_ranking_10k_h6_e3_lgbm_engmetric_clean/E3_event_hybrid_slots.fp16.npy \
  --loaded-slot-name E3_h6_cached \
  --skip-oracle \
  --skip-train-eval \
  --no-use-item-id-emb \
